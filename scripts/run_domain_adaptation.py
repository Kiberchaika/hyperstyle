...
class DomainAdaptation:
    def __init__(self, test_opts):
        ...
        self.restyle_opts = restyle_opts
        self.out_path_results = out_path_results
        self.out_path_coupled = out_path_coupled
        
        
    def process(self, finetuned_generator_checkpoint_path):
        finetuned_generator = load_generator(finetuned_generator_checkpoint_path)
        ...
        for input_batch in tqdm(self.dataloader):
            ...
            with torch.no_grad():
                ...
                result_batch, _ = run_domain_adaptation(input_cuda, self.net, self.opts, finetuned_generator,
                                                        self.restyle_e4e, self.restyle_opts)

            resize_amount = (256, 256) if self.opts.resize_outputs else (self.opts.output_size, self.opts.output_size)
            for i in range(input_batch.shape[0]):

                im_path = self.dataset.paths[global_i]

                curr_result = tensor2im(result_batch[i])
                input_im = tensor2im(input_batch[i])

                res_save_path = os.path.join(self.out_path_results, os.path.basename(im_path))
                curr_result.resize(resize_amount).save(res_save_path)

                coupled_save_path = os.path.join(self.out_path_coupled, os.path.basename(im_path))
